...
date: "Dernière mise à jour : été 2025"
output: 
  rmdformats::robobook:
    highlight: kate
...
<!-- # Chunk permettant d'utiliser les données S3. -->
...
<!-- L'équivalent du chargement des données en utilisant la library `aws.s3` (uniquement depuis une session SSPCloud) :  -->
...
st_crs(depCouronne_sf)$epsg
st_crs(paris_sf)$epsg
...
sfBase_sample <- sfBase[sample(1:nrow(sfBase), 2000), ]
...
- en rouge, 2000 points tirés au sort dans la base initiale : on enlèvera les points qui sont au-delà de la zone jaune.
...
mf_base(bbox_sf$geometry, col = "green", add = T)
mf_base(paris_sf, col = "blue", add = T)
mf_base(sfBase_sample$geometry, col = "red", add = T)
```
...
url_file <- url(paste0(url_bucket, "/sfCarLiss_quantiles.RDS"))
