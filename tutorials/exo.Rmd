---
title: "Formation au carroyage et lissage spatial sur R [EXERCICES] "
...
Pour cela, on utilise un jeux de données extrait d'un source *open-data* : les parkings publics géolocalisés dans dans l'agglomération bordelaise. Les données ont été téléchargées sur le site de l'[open-data de Bordeaux Métropole](https://opendata.bordeaux-metropole.fr/explore/dataset/st_park_p/export/?location=14,44.83244,-0.57022&basemap=jawg.streets) et ont été retravaillées pour les besoins de l'exercice (voir le script [suivant](https://github.com/InseeFrLab/formation-r-lissage-spatial/blob/main/tutorials/prepa_data.R) pour plus de détails).
...
Vous pouvez maintenant faire l'exercice en ouvrant un nouveau script vierge (CTRL+MAJ+N).
...
Pour cette partie, allez chercher directement la solution ci-dessous (hors programme de cette formation).
...
Pour information, `mapview` est un package de cartographie produisant des résultats proches de `leaflet`. Il est un peu plus rapide d'utilisation quant il s'agit de produire des résultats simples : notamment, il reprojette automatiquemeent les objets géograpahiques que vous souhaitez cartographier. Vous trouverez davantage d'informations à propos [ici](https://r-spatial.github.io/mapview/).
...
- Transformer la base des parkings en format vectoriel et enregistrer la résultat dans la nouvelle variable `sfparkings`:
...
- Créer la table `centroides` des centroïdes des carreaux de 1000m :
  ...
  - En faisant des "modulo 1000" sur les coordonnées x et y (comme dans le tutoriel)
...
À chaque parking est maintenant associé le couple x_centroide et y_centroide du carreau de 1km auquel le parking appartient.
En groupant la base de parkings sur le couple de centroides, on aboutit à la base souhaitée.
```
...
# Restriction du champ : possibilité 1
# on ne retient que les carreaux intersectant Bordeaux-centre
sfcarreaux <- sfcarreaux %>% st_join(centreBdx,left = F)

# # Restriction du champ : possibilité 2
# # on ne retient que les carreaux intersectant Bordeaux-centre
...
- Sélectionner les parkings positionnés dans cette zone tampon et enregistrer la nouvelle table `parkingsBuff`. Attention : cette nouvelle table doit être issue de la table `parkings` non vectorielle pour garder des colonnes x et y en vue du carroyage.
```


```{exo6clue, opts.label = "clues"}
- Créer un "buffer" du centre-ville de Bordeaux (avec une marge de 3 000 mètres autour) et enregistrer le résultat dans `centreBdxBuffer` ==> Utiliser la fonction `sf::st_buffer`
- Sélectionner les parkings (de la table vectorielle `sfparkings`) intersectant cette zone tampon et enregistrer la nouvelle table `parkingsBuff` ==> Utiliser la fonction `sf::st_contains` ou `sf::st_intersects`
...
- Restreindre les carreaux de 50 mètres obtenus aux seuls intersectant le centre-ville et enregistrer le résultat dans `sfcarLisse`
- Cartographier le résultat obtenu avec la fonction `genercarte` (chargée dans votre environnement global)

Sous-question bonus : que représente la variable `nbObsLisse` de `sfcarLisse` ? Proposer une transformation permettant d'obtenir une densité au km². 
```
...
# Comment afficher une densité au km² ?
sfcarLisse$nbObsLisse <- sfcarLisse$nbObsLisse*((1000/tcar)^2)

# Même carte lissée avec une légende au km²
genercarte(sfcarLisse=sfcarLisse,
                       nomvar="nbObsLisse",
                       nbreaks=5,
                       zone=centreBdx,
...
Refaire le lissage avec un rayon de 600 mètres, puis avec un rayon de 3000 mètres
...
# Avec un rayon de 3000m
rayon <- 3000 # Rayon de lissage
...
Pour la suite, toujours prendre un rayon de lissage de 1 700 mètres sauf mention contraire.
```
