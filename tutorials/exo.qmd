...
packages <-  c("dplyr", "sf", "mapsf", "mapview", "btb")
...
parkings <- readRDS(url(paste0(url_bucket, "parking.RDS")))
centreBdx <- st_read(paste0(url_bucket, "bordeaux_centre.gpkg"))
```
...
```{r generCarte}
generCarte <- function(sfCar, nomvar, nbreaks = 5, zone, titre = "titre"){
...
centroides <- centroides %>% 
  group_by(x_centro, y_centro) |> count(name = "nbParkings")
head(centroides)
```
:::
...
3. Cartographie les carreaux et le contour du centre de Bordeaux avec `generCarte`.
```
:::
...
1. Comme dans le tutoriel, charger la base des données Filosofi carroyées (grille de 200m) en ne chargeant que les carreaux de la Gironde (département 33) grâce à l'option `query` de `st_read`.
...
# 1. Chargement des données de Filosofi en Gironde
url_bucket <- "https://minio.lab.sspcloud.fr/projet-formation/r-lissage-spatial/"
