...
Nous allons charger le contour géographique des départements de la petite couronne parisienne. En cartographie, ces fichiers sont appelés des «\ couches vectorielles\ » (à ne pas confondre avec les couches «\ raster\ » qui correspondent, pas exemple, aux fonds de carte satellite OpenStreetMap ou Google Maps). 
...
Récupérons donc le contour du territoire étudié avec la fonction `read_sf` du package `sf`.
...
Ci-dessous la grille carroyée obtenue (hors données lissées). On remarque épouse le périmètre des points en entrée de la fonction. Par défaut, elle va même un peu au-delà (voir partie 4.1.5).
...
```{r verifObsQuantiles, eval=FALSE}
...
2. Créer un cercle de 1km autour du White
