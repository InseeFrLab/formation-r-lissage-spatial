...
- Code de la formation : [https://github.com/InseeFrLab/formation-r-lissage-spatial](https://github.com/InseeFrLab/formation-r-lissage-spatial)
- Site web des supports de formation : [https://inseefrlab.github.io/formation-r-lissage-spatial](https://inseefrlab.github.io/formation-r-lissage-spatial)
...
![](../img/lightsemio.png) *Auteur : Timothée Giraud, auteur de la librairie `mapsf`*
...
Pour ce TP, nous utilisons la base « [**D**emandes de **V**aleurs **F**oncières](https://www.data.gouv.fr/fr/datasets/demandes-de-valeurs-foncieres-geolocalisees/) » (DVF), qui recense l'ensemble des ventes de biens fonciers réalisées au cours des cinq dernières années (hors Mayotte et Alsace-Moselle). Les biens concernés peuvent être bâtis (appartement et maison) ou non bâtis (parcelles et exploitations). Les données sont produites par la Direction Générale des Finances Publiques (DGFiP). Elles proviennent des actes enregistrés chez les notaires et des informations contenues dans le cadastre.
...
3.  Passer d'une table de centroïdes à une table de carreaux vectoriels grâce à la fonction `btb_ptsToGrid` qui attend comme paramètres :
...
2.  Dans le second cas, on lisse uniquement les ventes réalisées dans **Paris intramuros** pour observer les effets de bord (attention : les seuils des classes sont légérement différents).
...
- ... mais aussi les 4 carreaux limitrophes de ces derniers (règle automatique[^1])

[^1]: cf. séquence "théorie du lissage" de la formation "Comment utiliser les outils de l’analyse urbaine ?"
...
Dans la continuité du précédent exercice, nous calculons le nombre et le prix moyen des transactions immobilières en 2021 dans le "triangle d'or[^2]", situé dans le 8e arrondissement de Paris. Ce quartier est connu pour ses immeubles cossus, ses hôtels et ses commerces de luxe.

[^2]: Pour information, ce triangle a été créé manuellement sur le site du [Géoportail](https://www.geoportail.gouv.fr/).
...
st_read_maison <- function(chemin_tab){
  requete <- "SELECT idcar_200m, lcog_geo, i_est_200, ind, men, log_soc, geom
            ...
            WHERE SUBSTR(lcog_geo, 1, 2) IN ('75','91','92','93')"
...
<!-- note : le support de formation tourne assez vite grâce à la requête -->
<!-- mais pour l'intégration continue, c'est nécessaire de simplifier -->

2.  Créer un disque de 1 km autour du White
...
3.  Déterminer, pour ce disque, les carreaux qui le recouvrent
...
Visualisation des carreaux issus de Filosofi carroyé 2019 qui intersectent notre zone d'intérêt :
...
4.  Calculer l'indicateur sur la zone par somme des données carroyées.
